...
def main(argv: Sequence[str] | None = None) -> None:  # pragma: no cover
    parser = argparse.ArgumentParser(description="Generate the PH121 assignment report")
    parser.add_argument(
        "--figures",
        type=Path,
    ...
    if args.figures is not None or args.show_figures:
        try:
            from .visuals import render_report_figures

            saved = render_report_figures(report, output_dir=args.figures, show=args.show_figures)
        except (RuntimeError, ValueError) as exc:
            print(f"Failed to produce figures: {exc}", file=sys.stderr)
        else:
            if saved:
                print("Saved figures:")
                for path in saved:
